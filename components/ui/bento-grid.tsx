...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText("ariful6@yahoo.com");
    setCopied(true);
  };
  ...
  return (
    ...
    >
      <div className={`${id === 6 && "flex justify-center"} h-full`}>
        ...
        <div
          className={`absolute -bottom-5 right-0 ${id === 5 && "opcity-80 w-full"}`}
        >
          {spareImg && (
            <Image
              width="100"
        ...
        >
            ...
            <div className="absolute -right-3 flex w-fit gap-1 lg:-right-2 lg:gap-5">
              <div className="flex flex-col gap-3 lg:gap-8">
                {["React.js", "Next.js", "Typescript"].map((item) => (
                  <span
                    key={item}
                    className="bg[#10132E] rounded-lg px-3 py-2 text-center text-xs opacity-50 lg:px-3 lg:py-4 lg:text-base lg:opacity-50"
                  >
                    {item}
                  </span>
                ))}
          ...
          {id === 6 && (
            <div className="relative mt-5">
              ...
              <MagicButton
                title={copied ? "Emial copied" : "Copy my email"}
                icon={<IoCopyOutline />}
                position="left"
                className="!bg-[#161a31]"
